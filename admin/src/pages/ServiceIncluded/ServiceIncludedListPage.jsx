...
const ServiceIncludedListPage = () => {
  ...
  const fetchServiceIncludes = async () => {
    try {
      setLoading(true);
      const response = await axios.get(apis.serviceIncluded.get, {
  ...
  const deleteServiceIncluded = async (id) => {
    ...
    try {
      const response = await axios.delete(`${apis.serviceIncluded.delete}/${id}`, {
        headers: { Authorization: validToken },
      });
...
export default ServiceIncludedListPage;
