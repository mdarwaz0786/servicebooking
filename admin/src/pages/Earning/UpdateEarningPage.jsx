...
const UpdateEarningPage = () => {
  ...
  useEffect(() => {
    const fetchEarning = async () => {
      try {
        const res = await axios.get(`${apis.earning.get}/${id}`, {
          headers: { Authorization: validToken },
        });
        if (res?.data?.success) {
          ...
          setFormData({
            ...
            earningHour1: e?.earningHour1 || "",
            earningPrice1: e?.earningPrice1 || "",
            earningHour2: e?.earningHour2 || "",
            earningPrice2: e?.earningPrice2 || "",
            earningHour3: e?.earningHour3 || "",
            earningPrice3: e?.earningPrice3 || "",
            earningHour4: e?.earningHour4 || "",
            earningPrice4: e?.earningPrice4 || "",
          });
        };
      } catch (err) {
        console.error(err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const data = { ...formData, updatedBy: user?._id };
      const res = await axios.patch(`${apis.earning.update}/${id}`, data, {
        headers: { Authorization: validToken },
      });
      if (res?.data?.success) {
      ...
      <div className="container mt-4 mb-5">
        <div className="card">
          <div className="card-header d-flex justify-content-between align-items-center">
            <h5 className="mb-0">Update Earning</h5>
          ...
          <div className="card-body">
            <form onSubmit={handleSubmit}>
              ...
              <div className="text-end">
                <button type="reset" className="btn btn-secondary me-2" onClick={resetForm} disabled={loading}>
                  Cancel
                </button>
...
export default UpdateEarningPage;
