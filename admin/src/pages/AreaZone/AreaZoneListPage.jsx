...
const AreaZoneListPage = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(apis.areaZone.get, {
  ...
  const deleteData = async (id) => {
    ...
    try {
      const response = await axios.delete(`${apis.areaZone.delete}/${id}`, {
        headers: { Authorization: validToken },
      });
      if (response?.data?.success) {
                  ...
                  {data.length > 0 ? (
                    data.map((d, index) => (
                      <tr key={d._id}>
                        ...
                        <td>{d.name}</td>
                        <td>{d.localityIds?.map((l) => <p>{l?.name}</p>)}</td>
                        <td>
...
export default AreaZoneListPage;
