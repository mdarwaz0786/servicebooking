...
const RateCardListPage = () => {
  ...
  const fetchRateCards = async () => {
    try {
      ...
      toast.error(error?.response?.data?.message || "Failed to fetch rate cards");
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateParams = (newParams) => {
    const params = { page, limit, search: debouncedSearch, sort, ...newParams };
  ...
  const deleteRateCard = async (id) => {
    ...
    try {
      const response = await axios.delete(`${apis.rateCard.delete}/${id}`, {
        headers: { Authorization: validToken },
      });
    ...
    <div className="page-wrapper page-settings">
      <div className="content">
        <div className="content-page-header content-page-headersplit mb-0 d-flex align-items-center justify-content-between">
          <h5>Rate Cards {rateCards?.length}</h5>
                      ...
                      <tr key={d?._id}>
                        ...
                        <td>
                          {d?.rateGroups?.length > 0 ? (
                            d.rateGroups.map((group, index) => (
                              <div key={index}>{group?.title || "Untitled"}</div>
                            ))
                          ) : (
                        ...
                        <td>
                          <div className="d-flex">
                            <Link to={`/update-rate-card/${d?._id}`}>
