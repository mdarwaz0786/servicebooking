...
const CombinedZoneListPage = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(apis.combinedZone.get, {
  ...
  const deleteData = async (id) => {
    ...
    try {
      const response = await axios.delete(`${apis.combinedZone.delete}/${id}`, {
        headers: { Authorization: validToken },
      });
      if (response?.data?.success) {
    ...
    <div className="page-wrapper page-settings">
      <div className="content">
        <div className="content-page-header content-page-headersplit mb-0 d-flex align-items-center justify-content-between">
          <h5>Combined Zones {data?.length}</h5>
                  ...
                  {data?.length > 0 ? (
                    data?.map((d, index) => (
                      <tr key={d._id}>
                        ...
                        <td>{d?.name}</td>
                        <td>{d?.zones?.map((zone) => zone?.name)?.join(", ")}</td>
                        <td>
                        ...
                        <td>
                          <div className="d-flex">
                            <Link to={`/update-combined-zone/${d?._id}`}>
...
export default CombinedZoneListPage;
